...
export default function MainSection(props) {
  ...
  useEffect(() => {
    ...
    onSnapshot(q, (snapshot) => {
      const temp_data = snapshot.docs.map((doc) => doc.data());
        ...
        <Box style={{ paddingTop: 37 }}>
          <Grid container spacing={5}>
            {productImageData.map((ele, i) => {
              ...
              if (ele.item.show_state === true)
                ele.display_style = classes.customize_button_container;
              else if (ele.item.show_state === false)
                ele.display_style = classes.customize_button_container_hidden;
              return <ProductImageBar key={i} {...ele} />;
            })}
          </Grid>
        </Box>
        <StatisticsBar iconData={iconData} />
        <BottomImageBar
          bottomBannerImageData={bottomBannerImageData}
